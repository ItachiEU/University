...
unordered_map<int, pair<pair<int, int>, int>> getDistAndPath(int start_x, int start_y, State *game)
{
   unordered_map<int, pair<pair<int, int>, int>> best_per_type; // type -> {dest, distance}
   for (int i = -2; i < 15; i++)
      best_per_type[i] = {{-1, -1}, 1000};
   ...
   while (!q.empty())
   {
      ...
      {
         if (best_per_type[game->entity_info[t.second.second][t.second.first].second.type].second > t.first)
         {
            while (parents[ending.second][ending.first] != make_pair(-1, -1))
            {
               path.push_back(ending);
            ...
            best_per_type[game->entity_info[t.second.second][t.second.first].second.type].second = t.first;
         ...
         if (!game->visited.count({t.second.first, t.second.second}) and game->entity_info[t.second.second][t.second.first].second.type != EXIT and best_per_type[EMPTY].second > t.first)
         {
            cerr << "entered" << endl;
            if (path.empty())
            {
            ...
            // if(count_uknown_fields(t.second.first,t.second.second) > 2){
            //     cerr << "considering: " << t.second.first << " " <<  t.second.second << endl;
            best_per_type[EMPTY].second = t.first;
...
Move flatMC(State *gameState, vector<Move> validMoves, chrono::steady_clock::time_point time_start)
{
   ...
   while (chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now() - time_start).count() < TIME_THRESHOLD)
   {
      ...
      for (int i = 0; i < validMoves.size(); i++)
         if (validMoves[i].type == 'a' and validMoves[i].weapon == SWORD)
         {
            moveIndex = i;
            break;
         }
      ...
      simulateNMoves(&gameCopy, move, 30, time_start);
...
Move heuristic(State *game, vector<Move> moves, chrono::steady_clock::time_point time_start)
{
   int dmg = 0;
   Move chosenMove;
   cerr << "before attacks: " << chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now() - time_start).count() << endl;
   for (auto move : moves) // most dmg
   {
      ...
      if (dealing > dmg) // most dmg
      {
         ...
         chosenMove = move;
      else if (dealing == dmg and getViewDistance({gameCopy->player_x, gameCopy->player_y}, {move.x, move.y}) <= attackRange[game->entity_info[move.y][move.x].first.type]) // priority on the one hitting us
         chosenMove = move;
      else if (dealing < dmg and getViewDistance({gameCopy->player_x, gameCopy->player_y}, {move.x, move.y}) <= attackRange[game->entity_info[move.y][move.x].first.type] and game->entity_info[move.y][move.x].first.HP <= dealing) // priority on kills
         chosenMove = move;
      delete gameCopy;
   }
   cerr << "before whole: " << chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now() - time_start).count() << endl;
   if (chosenMove.x == -1) // nearest treasure
   {
      int dist = 1000;
      pair<int, int> dest = {-1, -1};
      bool potion_not_found = false;

      unordered_map<int, pair<pair<int, int>, int>> best_per_type = getDistAndPath(game->player_x, game->player_y, game); // type -> {dest, distance}
      cerr << "after whole: " << chrono::duration_cast<chrono::milliseconds>(chrono::steady_clock::now() - time_start).count() << endl;
      if (game->player_HP <= 9) // low hp, go for potion
      {
         dist = best_per_type[POTION].second;
      ...
      {
         dist = best_per_type[TREASURE].second;
      ...
      if (dist == 1000 or potion_not_found or (best_per_type[EXIT].second != 1000 and best_per_type[EXIT].second > 150 - (game->turn + 10))) // nearest exit
      {
         dist = best_per_type[EXIT].second;
      ...
      if (dist == 1000) // nearest unvisited field
      {
         dist = best_per_type[EMPTY].second;
      ...
      if (dist != 1000 and chosenMove.type == 'm')
      {
         ...
         if (game->player_HP > gameCopy->player_HP)
         {
            for (auto move : moves) // most dmg
            {
               ...
               if (gameCopy2->entity_info[move.y][move.x].first.type == BOX and move.weapon != SWORD and !BOX_LEVEL)
                  continue;
               int dealing = gameCopy2->applyPlayerMove(move);
