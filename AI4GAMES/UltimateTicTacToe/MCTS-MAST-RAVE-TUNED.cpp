...
double ROUND_TIME = 95.0;     // miliseconds
double FIRST_ROUND = 995.0;   // miliseconds
...
NodeWithMove bestChild(Node *node, int currentPlayer, UltimateTicTacToe &game, vector<pair<int, int>> &validMoves)
{
   ...
   {
      double score = (double)(son.second->wins[currentPlayer] / (double)son.second->simulations) + 0.4 * sqrt((2.0 * log(node->simulations) / son.second->simulations));
      double K = 800;
...
int main()
{
   ...
   while (1)
   {
      ...
      for (auto &move : MAST)
      {
         move.second = {move.second.first * 0.8,
                        move.second.second * 0.8};
      }
   }
}