...
vector<string> usedMoves; // temporary
...
NodeWithMove bestChild(Node *node, int currentPlayer, UltimateTicTacToe &game, vector<pair<int, int>> &validMoves)
{
   ...
   {
      double score = (double)(son.second->wins[currentPlayer] / (double)son.second->simulations) + 1.4 * (2 * log(node->simulations) / son.second->simulations);
      if (score > bestScore)
      {
