...
(provide eval-while parse-while env-empty env-lookup eval-env)
; --------- ;
; Wyrazenia ;
; --------- ;
    ...
    [(letrec-expr x e1 e2)
     (let* ([new-env (env-add x (blackhole) env)]
       ...
       (begin
          ...
          (eval-env e2 new-env)))]
    ;[(var-expr x) (let ((found (env-lookup x env)))
                    ;(if (lam? found)
                        ;(eval-env found env)
                        ;found))]
    [(var-expr x) (let ((found (env-lookup x env)))
...
(define (eval-while e env)
  (match e
    [(skip) env]
    [(assign x e)
       (let [(tmp (eval-env e env))]
         (if (clo? tmp)
             (env-update x e env)
             (env-update x tmp env)))]
    [(if-cmd eb ct cf)
     (if (eval-env eb env)
         (eval-while ct env)
         (eval-while cf env))]
...
(define (wykonaj co)
  (let* ([init-env env-empty]
         [final-env
          (eval-while (parse-while co) init-env)])
    (env-lookup 'z final-env)))
(define prog7
  '{(x := 5)
   (f := (let (w 5) (lambda (y) (+ x y))))
   (x := 10)
   (z := (f 0))})
