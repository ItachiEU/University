...
def evaluate(positions, goals, odleglosci, stopien):

    result = 0
    odl = []
    for pos in positions:
        odl.append(odleglosci[pos[0]][pos[1]])
    odl.sort(reverse=True)
    for i in range(min(len(odl), stopien)):
        result += odl[i]
    return result
    ...
    while not q.empty():
        ...
        for ruch in moves:
            ...
            if tuple(temp) not in visited:
                # zmieniajac stopien z 1 na wyzsza liczbe przyspieszamy, algorytm ale uzyskujemy dluzsze (nieptymalne) odpowiedzi
                val = evaluate(temp, goals, odleglosci, 2)
                d = len(path)
                counter += 1
