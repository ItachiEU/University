...
while [ 1 ]
do
    CHOICE=$(whiptail --title "Music Player" --menu "Choose a song to play" 15 100 9 "${args[@]}" 3>&1 1>&2 2>&3)
    if [[ $CHOICE == "Search by title" ]]; then
        SEARCH=$(whiptail --title "Music Player" --inputbox "Type title" 15 100 "" 3>&1 1>&2 2>&3)
        args=("Search by title" "")
        index=0
        for item in "${tracks[@]}"; do
            index=$((index+1))
