...
class MyCellRender extends DefaultListCellRenderer {
   ...
   public Component getListCellRendererComponent(JList<?> list, Object value, int index, boolean isSelected,
      ...
      if (value.toString().contains("Sunday")) {
         Color fg = Color.RED;
         setForeground(fg);
      } else {
...
public class MyCalendar extends javax.swing.JFrame {
   ...
   GregorianCalendar globalCalendar;
   ...
   public MyCalendar() {
      ...
      currYear = globalCalendar.get(Calendar.YEAR);
      currMonth = globalCalendar.get(Calendar.MONTH);
      currDay = 1;

      initComponents();
      JTabbedPane tabbedPane = (JTabbedPane) SwingUtilities.getAncestorOfClass(JTabbedPane.class, yearPanel);
      tabbedPane.setTitleAt(0, "Year " + currYear);
   }
   ...
   MouseListener ml = new MouseAdapter() {
      ...
      public void mousePressed(MouseEvent e) {
         ...
         currMonth = map.get(label.getText());
         globalCalendar = new GregorianCalendar(globalCalendar.get(Calendar.YEAR), currMonth, 1);
         populateLists();
      }
   };
   ...
   void populateMonthPane(JPanel currMonthPanel, String name) {
      ...
      currMonthPanel.removeAll();
      // if(globalCalendar.get(Calendar.YEAR) > 1582)
      GregorianCalendar calendar = new GregorianCalendar(globalCalendar.get(Calendar.YEAR), map.get(name), 1);
      ...
      if (calendar.get(GregorianCalendar.YEAR) == 1582 && name == "October") {
         daysInMonth = 21;
         firstDay = 1;
      }

      currMonthPanel.setBorder(BorderFactory.createLineBorder(new Color(0, 0, 0)));
      currMonthPanel.addMouseListener(ml);
      ...
      if (calendar.get(GregorianCalendar.YEAR) == 1582 && name == "October")
         for (int i = 1; i <= 42; i++) {
            JLabel day;
            if (i - firstDay >= 4 && !changed) {
               firstDay -= 10;
               changed = true;
            }
            if (i < firstDay || i - firstDay >= 31)
               day = new JLabel("");
            else {
               if (i % 7 == 0) {
                  JLabel temp = new JLabel(Integer.toString(i - firstDay + 1));
                  temp.setForeground(Color.red);
                  day = temp;
               } else
                  day = new JLabel(Integer.toString(i - firstDay + 1));
            }
            daysPanel.add(day);
         }
      else
   ...
   void populateOneList(JList lista, GregorianCalendar calendar) {
      ...
      String[] listData;
      if (y == 1582 && month.contains("Oct")) {
         daysInMonth = 21;
         listData = new String[daysInMonth];
         int firstDay = 1;
         boolean changed = false;
         JLabel day;
         int j = 0;
         for (int i = 1; i <= 42; i++) {
            ...
            if (i < firstDay || i - firstDay >= 31)
               ...
               if (i % 7 == 0) {
                  listData[j] = "Sunday " + Integer.toString(i - firstDay + 1) + " Oct";
                  j++;
               } else {
                  day = new JLabel(Integer.toString(i - firstDay + 1));
                  listData[j] = daysFull[(7 + i - 1) % 7] + " " + Integer.toString(i - firstDay + 1) + " Oct";
                  j++;
               }
            }
         ...
         for (int i = 0; i < daysInMonth; i++) {
            ...
            listData[i] = dayOfWeek + " " + Integer.toString(i + 1) + " " + month;
   ...
   void populateLists() {
      GregorianCalendar calendar = globalCalendar;

      SimpleDateFormat monthFormat = new SimpleDateFormat("MMM");
      String month = monthFormat.format(calendar.getTime());
      JTabbedPane tabbedPane = (JTabbedPane) SwingUtilities.getAncestorOfClass(JTabbedPane.class, yearPanel);
      tabbedPane.setTitleAt(1, "Month " + month);

      populateOneList(currMonthList, calendar);
      populateOneList(nextMonthList, calendar);
      calendar.add(GregorianCalendar.MONTH, -3);
      populateOneList(prevMonthList, calendar);
   }
   ...
   private void initComponents() {
      ...
      prevMonthList = new javax.swing.JList<String>();
      currMonthPane = new javax.swing.JScrollPane();
      currMonthList = new javax.swing.JList<String>();
      nextMonthPane = new javax.swing.JScrollPane();
      nextMonthList = new javax.swing.JList<String>();
      ...
      yearInput.addActionListener(new ActionListener() {
         public void actionPerformed(ActionEvent e) {
            ...
            globalCalendar = new GregorianCalendar(value, globalCalendar.get(Calendar.MONTH), 1);
            populateLists();
            JTabbedPane tabbedPane = (JTabbedPane) SwingUtilities.getAncestorOfClass(JTabbedPane.class, yearPanel);
            tabbedPane.setTitleAt(0, "Year " + value);
            for (int i = 0; i < 12; i++) {
      ...
      monthPanel.add(yearInput, gridBagConstraints);
